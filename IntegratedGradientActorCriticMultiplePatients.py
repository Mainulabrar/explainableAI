...
for patient in test_set:
    ...
    for i in range(repSize-1):
        ...
        attributions, delta = ig.attribute(inputs= (state, baseh), baselines= (baselineTensor, baseh), target = actionPolicy, return_convergence_delta=True)
        ...
        # # print('adding Up baseline and summed all the feature contributions of IG values', output.detach().numpy()[0, actionPolicy2nd]+ np.sum(attributions.detach().numpy(), axis = 1))
        
        # prediction = output.detach().numpy()[0, actionPolicy]+ np.sum(attributions.detach().numpy(), axis = 1)
        # # prediction = output.detach().numpy()[0, actionPolicy2nd]+ np.sum(attributions.detach().numpy(), axis = 1)
        
        # print('Mannually Calculated delta', -model(state).detach().numpy()[0, actionPolicy] + prediction)
        # # print('Mannually Calculated delta', -model(state).detach().numpy()[0, actionPolicy2nd] + prediction)
        # print('Policy2ndAction', model(state).detach().numpy()[0, actionPolicy2nd])
        # print('Policy1stAction', model(state).detach().numpy()[0, actionPolicy])
