...
for patient in test_set:
    ...
    for j in range(repSize-1):
	    Y = np.load(f"/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/dataWithPlanscoreRun/{patient}xDVHY120499step{j}.npy")
	    IGforColor = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/{patient}attributionStateRep{j}.txt')
	    ...
	    maxColorArray.append(max(IGforColor.max(), IG2ndforColor.max()))
	    minColorArray.append(min(IGforColor.min() ,IG2ndforColor.min()))
    ...
    for i in range(repSize-1):
	    ...
	    actionPolicy = np.argmax(np.load(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/dataWithPlanscoreRun/{patient}Policy120499step{i}.npy'))
	    actionPolicyPolicyvalue = np.max(np.load(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/dataWithPlanscoreRun/{patient}Policy120499step{i}.npy'))
	    Policy = np.load(f"/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/dataWithPlanscoreRun/{patient}Policy120499step{i}.npy")
	    ...
	    IGvalues = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/{patient}attributionStateRep{i}.txt')
	    IGhx = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/{patient}attributionhxRep{i}.txt')
	    IGcx = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/{patient}attributioncxRep{i}.txt')   
	    ...
	    TotalPreviousStateContribution = np.sum(IGhx)+ np.sum(IGcx)
	    print('action', action)
		    		# if i == 0:
		# 	print(color[i])
	    ...
	    background = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/backgroundForIGLSTM.txt')
	    plt.title(f'Patient {patient} step:{i} actionPolicy{actionDictionary[action]} policy{actionPolicyPolicyvalue:.4f} Previous{TotalPreviousStateContribution}\nbackground {background[action]:.4f}, IGSumCheck {SumIGvalues:.4f}, AbsoluteSum {absoluteSumIGvalues:.4f}')
	    ...
	    plt.savefig(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/figures/{patient}IGValuesLSTMGraphCombinedStep{i}.png', dpi= 1200)
