...
spread = np.load('/data2/mainul/ExplainableAIResultsAllSteps/dataWithPlanscoreRun/perturbationSpreadAll.npy')

spread[spread == 0] = 1e-10
...
for i in range(color.shape[0]):
	...
	# ColorPTV = np.sum(color[i,0: 100])
	# ColorBladder = np.sum(color[i, 100: 200])
	# ColorRectum = np.sum(color[i, 200: 300])

	# ColorPTV = np.full(100, ColorPTV)
	# ColorBladder = np.full(100, ColorBladder)
	# ColorRectum = np.full(100, ColorRectum)	

	SpreadPTV = spread[i,0: 100]
	SpreadBladder = spread[i, 100: 200]
	SpreadRectum = spread[i, 200: 300]

	ColorPTV = ColorPTV/SpreadPTV
	ColorBladder = ColorBladder/SpreadBladder
	ColorRectum = ColorRectum/SpreadRectum
	...
	# vmin = min(ColorPTV, ColorBladder, ColorRectum)
	# vmax = max(ColorPTV, ColorBladder, ColorRectum)
	...
	# plt.savefig(f'/data2/mainul/ExplainableAIResultsAllSteps/figures/0ValueSaliency{i}fulline.png', dpi= 1200)
	# plt.savefig(f'/data2/mainul/ExplainableAIResultsAllSteps/figures/0focusedSaliency{i}fullline.png', dpi= 1200)
	# plt.savefig(f'/data2/mainul/ExplainableAIResultsAllSteps/figures/0focusedSaliency{i}.png', dpi= 1200)
	# plt.savefig(f'/data2/mainul/ExplainableAIResultsAllSteps/figures/0ValueSaliency{i}AdjustedPerturbedFullline.png', dpi= 1200)
	plt.savefig(f'/data2/mainul/ExplainableAIResultsAllSteps/figures/0ValueSaliency{i}AdjustedPerturbed.png', dpi= 1200)
	# plt.show()
	plt.close()
