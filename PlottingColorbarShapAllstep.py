...
for i in range(StepNumIndicator.shape[0]):
	...
	# if i == 0:
	# 	print(color[i])
	shapValues = np.load(f'/data2/mainul/ExplainableAIResultsAllSteps/dataWithPlanscoreRun/0ShapValuestep{i}.npy')
	color = shapValues[0,:, action[0]]
	...
	plt.savefig(f'/data2/mainul/ExplainableAIResultsAllSteps/figures/0ShapGraph{i}.png', dpi= 1200)
	# plt.show()
	plt.close()
