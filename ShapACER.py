            ...
            for patientid in range(args.evaluation_episodes):
                ...
                while True:
                    ...
                    if done:
                        ...
                        tPTV = TPPs['l1'][episode_length]
                        tBLA = TPPs['l2'][episode_length]
                        tREC = TPPs['l3'][episode_length]
                        lambdaPTV = TPPs['l4'][episode_length]
                        lambdaBLA = TPPs['l5'][episode_length]
                        lambdaREC = TPPs['l6'][episode_length]
                        VPTV = TPPs['l7'][episode_length]
                        VBLA = TPPs['l8'][episode_length]
                        VREC = TPPs['l9'][episode_length]

                        xVec = np.ones((MPTV.shape[1],))
                        gamma = np.zeros((MPTV.shape[0],))

                        planscoresSavePath = '/data2/mainul/ExplainableAIResultsShap/planscores/'
                        os.makedirs(planscoresSavePath, exist_ok = True)
                        data_result_path='/data2/mainul/ExplainableAIResultsShap/dataWithPlanscoreRun/'
                        os.makedirs(data_result_path, exist_ok = True)
                        actionSavePath = '/data2/mainul/ExplainableAIResultsShap/actions/'
                        ...
                        # planscoresSavePath = '/data2/mainul/ExplainableAIResultsPreTesting/planscores/'
                        # os.makedirs(planscoresSavePath, exist_ok = True)
                        # data_result_path='/data2/mainul/ExplainableAIResultsPreTesting/dataWithPlanscoreRun/'
                        # os.makedirs(planscoresSavePath, exist_ok = True)
                        # actionSavePath = '/data2/mainul/ExplainableAIResultsPreTesting/actions/'
                        # os.makedirs(actionSavePath, exist_ok = True)
                        np.save(data_result_path+str(patientid)+'xDVHYfull' + str(epoch)  + 'step' + str(episode_length),
                            Y)
                    ...
                    with torch.no_grad():
                        policy = model(state_test)

                    background = backgroundForShap(args)
                    ...
                    print('shap_values')
                    np.save(data_result_path+'0shapValues'+str(episode_length), shap_values)
                    ...
                    np.save(data_result_path+str(patientid)+'xDVHYfull' + str(epoch)  + 'step' + str(episode_length + 1),
                            Y)
...
parser.add_argument('--max-episode-length', type=int, default=2, metavar='LENGTH', help='Maximum episode length')
parser.add_argument('--hidden-size', type=int, default=32, metavar='SIZE', help='Hidden size of LSTM cell')
...
if __name__ == '__main__':
  with open('patient_test_CORS_30indCriTime.txt', 'w') as f:
