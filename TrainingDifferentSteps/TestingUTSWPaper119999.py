...
def test(rank, args, T):
    ...
    patient_list = ['014', '015', '023', '073', '098']
    # patient_list = ['098']
    # patient_list = ['080', '081', '083', '084', '085', '087', '088', '091', '092', '093', '095', '097', '098', '099']
    ...
    # =================This is the dataset for all UTSW data available in the backup drive=====================
    ...
    #     print("PTLabel loaded")

    # This block is for the new single utsw testing dataset =========================
    # for i in range(len(pid)):
    #     print("len(pid)", len(pid))
    ...
    # This block is for the new all utsw testing dataset =========================
    for i in range(len(pid)):
        print("len(pid)", len(pid))
        print('patient No', pid[i])
        globals()['doseMatrix_' + pid[i]] = loadDoseMatrix(data_path + pid[i] + '.hdf5')
        print("doseMatrix loaded")
        globals()['targetLabels_' + pid[i]], globals()['bladderLabel' + pid[i]], globals()[
          'rectumLabel' + pid[i]], \
        globals()['PTVLabel' + pid[i]] = loadMask(data_path2 + pid[i] + '.h5')
        print("PTVLabel loaded")
        print(globals()['doseMatrix_' + pid[i]].shape)
    ...
    while T.value() <= args.T_max:
        if can_test:
            ...
            model.load_state_dict(torch.load('/home/mainul/Actor-critic-based-treatment-planning/acer_VTPN-12-18-23/results/results/episode119999.pth'))
            model.eval()
            ...
            for patientid in range(len(pid)):
                ...
                while True:
                    ...
                    if done:
                        ...
                        planscoresSavePath = '/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingSteps/PaperInitLSTM119999/planscores/'
                        os.makedirs(planscoresSavePath, exist_ok = True)
                        data_result_path='/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingSteps/PaperInitLSTM119999/dataWithPlanscoreRun/'
                        ...
                        state_test0, iter, xVec = \
                            runOpt_dvh(MPTV, MBLA, MREC, tPTV, tBLA, tREC,
                                       ...
                                       VPTV, VBLA, VREC, xVec, gamma, pdose, maxiter)
                        # state_test0 = np.load(data_result_path+id1+'xDVHY' + str(epoch)  + 'step' + str(episode_length)+'.npy')
                        
                        # print(f'state{episode_length}', state_test0)
                        np.save(data_result_path+id1+'xDVHY' + str(epoch)  + 'step' + str(episode_length),
                            state_test0)
                        ...
                        planscoresSavePath = '/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingSteps/PaperInitLST119999/planscores/'
                        os.makedirs(planscoresSavePath, exist_ok = True)
                        data_result_path='/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingSteps/PaperInitLSTM119999/dataWithPlanscoreRun/'
                        os.makedirs(data_result_path, exist_ok = True)
                        np.save(data_result_path+id1+'xDVHYfull' + str(epoch)  + 'step' + str(episode_length),
                            Y)
                        ...
                        np.save(data_result_path+str(id1)+'Dose' + str(epoch)  + 'step' + str(episode_length), Dose)
                        np.save(data_result_path+str(id1)+'doseMatrix' + str(epoch)  + 'step' + str(episode_length), doseMatrix)
                        np.save(data_result_path+str(id1)+'bladderLabel' + str(epoch)  + 'step' + str(episode_length), bladderLabel)
                        np.save(data_result_path+str(id1)+'rectumLabel' + str(epoch)  + 'step' + str(episode_length), rectumLabel)
                        np.save(data_result_path+str(id1)+'PTVLabel' + str(epoch)  + 'step' + str(episode_length), PTVLabel)
                        ...
                        # logging.info("Iteration_num: {}  PlanScore: {}  PlanScore_fine: {}".format(iter, planScore,
                        #                                                                            planScore_fine))
                        ...
                        print(planScore)
                        np.save(planscoresSavePath+str(id1)+ 'planscoreInsideIf',planScore)
                        # ----------------two ways of NN schemes (with/without rules) --------------------------
                        state_test = state_test0.copy()
                        if cuda:
                    ...
                    print(f'state_test{episode_length}', state_test)
                    # Calculate policy
                    with torch.no_grad():
                    ...
                    hxSave = hx.numpy()
                    cxSave = cx.numpy()
                    # print('Policy', policySave)
                    np.save(data_result_path+str(id1)+'Policy' + str(epoch)  + 'step' + str(episode_length), policySave)
                    np.save(data_result_path+str(id1)+'hx' + str(epoch)  + 'step' + str(episode_length), hxSave)
                    np.save(data_result_path+str(id1)+'cx' + str(epoch)  + 'step' + str(episode_length), cxSave)

                    # Choose action greedily
                    # action = policy.max(1)[1][0]
                    # action = torch.multinomial(policy, 1)[0, 0]
                    TPPs = np.load(f"/data2/mainul/results_CORS1Paper/scratch6_30StepsNewParamenters3NewData1/dataWithPlanscoreRun/{id1}tpptuning120499.npz")

                    tPTV = TPPs['l1'][episode_length+1]
                    tBLA = TPPs['l2'][episode_length+1]
                    tREC = TPPs['l3'][episode_length+1]
                    lambdaPTV = TPPs['l4'][episode_length+1]
                    lambdaBLA = TPPs['l5'][episode_length+1]
                    lambdaREC = TPPs['l6'][episode_length+1]
                    VPTV = TPPs['l7'][episode_length+1]
                    VBLA = TPPs['l8'][episode_length+1]
                    VREC = TPPs['l9'][episode_length+1]
                    ...
                    state_test = n_state_test.copy()
                    # state_test = np.load(data_result_path+str(id1)+'xDVHY' + str(epoch)  + 'step' + str(episode_length + 1)+'.npy')
                    
                    # print(f'state{episode_length+1}',state_test)
                    # print

                    np.save(data_result_path+str(id1)+'xDVHY' + str(epoch)  + 'step' + str(episode_length + 1),
                            state_test)
                    if cuda:
                        state_test = torch.from_numpy(state_test).float()
                        state_test = state_test.to(device)
                    ...
                    np.save(data_result_path+str(id1)+'Dose' + str(epoch)  + 'step' + str(episode_length + 1), Dose)
                    ...
                    np.save(data_result_path+str(id1)+'xDVHYfull' + str(epoch)  + 'step' + str(episode_length + 1),
                            Y)
                    ...
                    np.save(planscoresSavePath+str(id1)+'planscoreBeforeWhileBreaking'+str(episode_length),planScore)
                    if planScore == 9:
                        done = True
                        break
                ...
                #         tpp_parameters)
                name1 = data_result_path + str(id1) + 'tpptuning' + str(epoch)
                ...
                np.save(planscoresSavePath+str(id1)+'planscoreBeforeForBreaking',planScore)
                # print(tPTV_all,tBLA_all, tREC_all, lambdaPTV_all,lambdaBLA_all,lambdaREC_all,VPTV_all,VBLA_all, VREC_all,planScore_all,planScore_fine_all)
...
async def send_notification():
    await bot.send_message(chat_id=CHAT_ID, text="The Process patient_test_UTSWall is Finished")

async def send_notification_error():
    await bot.send_message(chat_id=CHAT_ID, text="The Process patient_test_UTSWall is Finished with error")


# The following block is for running in the background==============================
if __name__ == '__main__':
  with open('patient_test_UTSWall.txt', 'w') as f:
...
# The following block is for live view ===================================
# if __name__ == '__main__':
...
#                 pass
  ...
  # This code is to check for multiprocessing
  # rank = 1
  # p = mp.Process(target=train2, args=(
  #   rank, args, T, shared_model, shared_average_model, optimiser))
  # p.start()
  # p.join()
  # train2(1, args, T, shared_model, shared_average_model, optimiser)
  ##################################################################

  # This code refers to limits of TPP adjustments as of 04-04-23
  ...
  #
  # if action == 0:
  #     tPTV = min(tPTV * increment_PTV, paraMax_tPTV)
  # elif action == 1:
  #     tPTV = max(tPTV * decrement_PTV, paraMin_tPTV)
  # elif action == 2:
  #     tBLA = min(tBLA * increment, paraMax_tOAR)
  # elif action == 3:
  #     tBLA = max(tBLA * decrement, paraMin)
  # elif action == 4:
  #     tREC = min(tREC * increment, paraMax_tOAR)
  # elif action == 5:
  #     tREC = max(tREC * decrement, paraMin)
  # elif action == 6:
  #     lambdaPTV = min(lambdaPTV * m.exp(PTV_e_increment), paraMax)
  # elif action == 7:
  #     lambdaPTV = max(lambdaPTV * m.exp(-PTV_e_increment), paraMin)
  # elif action == 8:
  #     lambdaBLA = min(lambdaBLA * m.exp(e_increment), paraMax)
  # elif action == 9:
  #     lambdaBLA = max(lambdaBLA * m.exp(-e_increment), paraMin)
  # elif action == 10:
  #     lambdaREC = min(lambdaREC * m.exp(e_increment), paraMax)
  # elif action == 11:
  #     lambdaREC = max(lambdaREC * m.exp(-e_increment), paraMin)
  # elif action == 12:
  #     VPTV = min(VPTV * increment_PTV, paraMax_VPTV)
  # elif action == 13:
  #     VPTV = max(VPTV * decrement_PTV, paraMin)
  # elif action == 14:
  #     VBLA = min(VBLA * increment, paraMax_VOAR)
  # elif action == 15:
  #     VBLA = max(VBLA * decrement, paraMin)
  # elif action == 16:
  #     VREC = min(VREC * increment, paraMax_VOAR)
  # elif action == 17:
  #     VREC = max(VREC * decrement, paraMin)

#
#
#
# ###########################################
# # Code description: Main file which combined other files
#
#
#
# ###########################################
