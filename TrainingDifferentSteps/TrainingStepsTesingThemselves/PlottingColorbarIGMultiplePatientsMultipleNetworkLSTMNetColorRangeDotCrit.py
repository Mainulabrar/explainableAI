...
for patient in test_set:
    ...
    for net in NetSet:
        StepNumIndicator = np.load(f"/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/dataWithPlanscoreRun/{patient}tpptuning120499.npz")
        repSize = StepNumIndicator['l1'].nonzero()[0].size
        repSizeArray.append(repSize)

    repSize = min(repSizeArray)
    repSizeArray = []
    ...
    for i in range(repSize-1):
        ...
        for net in NetSet:      
            IGforColor = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/{patient}attributionStateRep{i}.txt')
            # IG2ndforColor = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/PaperInitdebug/{patient}attributionStateRep2nd{i}.txt')
        ...
        minColorArray = []    	
        for net in NetSet:
            Y = np.load(f"/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/dataWithPlanscoreRun/{patient}xDVHY120499step{i}.npy")
            # if i != (repSize-1):
            ...
            actionPolicy = np.argmax(np.load(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/dataWithPlanscoreRun/{patient}Policy120499step{i}.npy'))
            actionPolicyPolicyvalue = np.max(np.load(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/dataWithPlanscoreRun/{patient}Policy120499step{i}.npy'))
            Policy = np.load(f"/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/dataWithPlanscoreRun/{patient}Policy120499step{i}.npy")
            ...
            IGvalues = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/{patient}attributionStateRep{i}.txt')
            IGhx = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/{patient}attributionhxRep{i}.txt')
            IGcx = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/{patient}attributioncxRep{i}.txt')   
            ...
            ProknowDoseBla = [0.812, 0.881, 0.943, 1.006]
            ProknowDoseRec = [0.755, 0.812, 0.881, 0.943]
            ProknowDosePTV = [1, 1.1]
            ProKnowVolumPTV = [0.95, 0.01]
            ProknowVolumeOAR = [0.55, 0.4, 0.3, 0.2]
            ...
            background = np.loadtxt(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/backgroundForIGLSTM.txt')
            plt.title(f'Patient {patient} step:{i} net {net} actionPolicy{actionDictionary[action]} policy{actionPolicyPolicyvalue:.4f} \nbackground {background[action]:.4f}, IGSumCheck {SumIGvalues:.4f}, CurrMem {CurrMem:.4f}, TotalMem {TotalMem}')
            ...
            plt.scatter(x_ptv, y_ptv, c = ColorPTV, vmin=vmin, vmax=vmax, cmap = 'jet', label = 'PTV')
            plt.scatter(x_bladder, y_bladder, c =  ColorBladder, vmin=vmin, vmax=vmax, cmap = 'jet', label='Bladder')
            plt.scatter(x_rectum, y_rectum, c = ColorRectum, vmin=vmin, vmax=vmax, cmap = 'jet', label='Rectum')

            plt.colorbar()

            plt.scatter(ProknowDoseBla, ProknowVolumeOAR, marker = '^', color = 'black')
            plt.scatter(ProknowDoseRec, ProknowVolumeOAR, marker = 's', color = 'black')
            plt.scatter(ProknowDosePTV, ProKnowVolumPTV, marker = '*', color = 'black')
            ...
            plt.annotate(
                ...
                xytext=(bladder_first_point[0] + 0.1, bladder_first_point[1] + 0.1),
                arrowprops=dict(facecolor='black', arrowstyle='->', shrinkA = 5, shrinkB=5, mutation_scale=10), fontsize=10
            )
            plt.annotate(
                'Rectum', xy=rectum_first_point, 
                xytext=(rectum_first_point[0] + 0.1, rectum_first_point[1] - 0.1),
                arrowprops=dict(facecolor='black', arrowstyle='->', shrinkA = 5, shrinkB=5, mutation_scale=10), fontsize=10
            )
            ...
            figureSavePath = f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/figuresJet/'
            os.makedirs(figureSavePath, exist_ok = True)
            plt.savefig(f'/data2/mainul/explainableAIResultsMultiplePatientCases/AllTrainingStepsThemSelves/PaperInitLSTM{net}/figuresJet/{patient}IGValuesLSTMGraphCombinedStep{i}.png', dpi= 1200)
